...
def total_unique(ll1, ll2, ll3):
    ...
    # ll2_child = dict()
    # ll3_child = dict()
    test_cases = dict() # testcase -> list_of_mutants
    all_mutants = set()

    for ll in [ll1, ll2, ll3]:
        for mutants, testcase in ll:
            match = re.search(r'do_.*_test .* \{\n((.|\n)*)\n\}', testcase)
            print(testcase)
            test_cases[match.group(1)] = mutants
            for mutant in mutants:
    ...
    while len(all_mutants) > 0:
        ...
        max_test = max(test_cases, key=lambda x: len([m for m in test_cases.get(x) if m in all_mutants]))

        llres.append(([m for m in test_cases[max_test] if m in all_mutants], max_test))
